...
//    }
    ...
    public List<Project> getProjectList(@PageableDefault(size = 60) Pageable pageable) {
        List<Project> result = projectService.getProjects(pageable).getContent();
        System.out.println("===========2번=======");
        return result;
    }

    @GetMapping("/test/lazy")
    @ApiOperation(value = "프로젝트 전체 조회 지연로딩테스트", notes = "프로젝트 전체 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(code = 200, message = "성공"),
            @ApiResponse(code = 401, message = "인증 실패"),
            @ApiResponse(code = 404, message = "사용자 없음"),
    ...
    public List<ProjectListTestRes> getProjectListTestLazy(@PageableDefault(size = 60) Pageable pageable) {
        return projectService.getProjectsTestLazy(pageable).getContent();
    }

    @GetMapping("/test/fetch")
    @ApiOperation(value = "프로젝트 전체 조회 페치조인테스트", notes = "프로젝트 전체 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(code = 200, message = "성공"),
            @ApiResponse(code = 401, message = "인증 실패"),
            @ApiResponse(code = 404, message = "사용자 없음"),
    ...
    public List<Project> getProjectListTestFetch(@PageableDefault(size = 60) Pageable pageable) {
        return projectService.getProjectsTestFetch(pageable).getContent();
    }

    @GetMapping("/test/fetch/mappedby")
    @ApiOperation(value = "프로젝트 전체 조회 mappedby테스트", notes = "프로젝트 전체 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(code = 200, message = "성공"),
            @ApiResponse(code = 401, message = "인증 실패"),
            @ApiResponse(code = 404, message = "사용자 없음"),
    ...
    public List<ProjectListTestFetchRes> getProjectListTestMappedBy(@PageableDefault(size = 60) Pageable pageable) {
        return projectService.getProjectsTestMappedBy(pageable).getContent();
    }
